        ...
        for alpha = 1:length(grid_size.Alpha)
            for sig = 1:length(grid_size.Sig)
                for rsp = 1:length(grid_size.Rsp)
                    vrsp = grid_size.Rsp(rsp) + grid_size.Rp(rp) * exp(-0.5*angdiff(data.angle-grid_size.Op(op)).^2/grid_size.Sig(sig)^2) + grid_size.Alpha(alpha)* grid_size.Rp(rp) * exp(-0.5*angdiff(data.angle-(grid_size.Op(op)+180)).^2/grid_size.Sig(sig)^2);
